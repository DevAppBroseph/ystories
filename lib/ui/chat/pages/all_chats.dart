...
class _AllChatsPageState extends State<AllChatsPage>
  ...
  final _chatController = StreamController<List<Chat>>();
  ...
  void messages() {
    ...
    _chatController.sink.addStream(query.snapshots().map(
      (snapshot) {
          ...
          builder: (context, AsyncSnapshot<List<Chat>> data) {
            return Column(
              children: [
                ...
                if (_searchChats == null)
                  Expanded(
                    flex: 1,
                    child: StreamBuilder(
                      // stream: _chatController.stream,
                      builder: (context, AsyncSnapshot<List<Chat>> data1) {
                        if (data.hasData) {
                          var chats = data.data
                              ?.where(
                                (element) =>
                          ...
                          if (usersInfo.length == _chats.length &&
                              !loading &&
                              _searchChats == null) {
                              ...
                              slivers: [
                                ...
                                SliverList(
                                  delegate: SliverChildBuilderDelegate(
                                    (context, index) => _chatCard(
                                      _chats[index],
                                    ),
                                    childCount:
                                        usersInfo.length == _chats.length
                                            ? usersInfo.length
                                            : 0,
                                  ),
                                ),
                ...
                if (_searchChats != null)
                  Expanded(
                      ...
                      child: CustomScrollView(
                        slivers: [
                          if (_searchChats!
                              .allUsersFromSubscriptions.isNotEmpty)
                          ...
                          SliverList(
                            delegate: SliverChildBuilderDelegate(
                              (context, index) => _searchCard(
                                  _searchChats!
                                      .allUsersFromSubscriptions[index],
                                  false),
                              childCount: _searchChats!
                                  .allUsersFromSubscriptions.length,
                            ),
                          ),
                          ...
                          SliverList(
                            delegate: SliverChildBuilderDelegate(
                              (context, index) => _searchCard(
                                  _searchChats!.allUsers[index], false),
