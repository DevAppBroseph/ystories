...
class _CameraPageState extends State<CameraPage> with WidgetsBindingObserver {
  ...
  void _setCamera(int? index) async {
    var front = cameras.firstWhere(
        (camera) => camera.lensDirection == CameraLensDirection.front);
    controller = CameraController(
      index != null ? cameras[index] : front,
    ...
    controller?.initialize().then((_) {
      ...
      setState(() {});
      print(controller?.value.deviceOrientation);
    });
  }
            ...
            children: [
              GestureDetector(
                ...
                onLongPressMoveUpdate: (details) {
                  if (details.localOffsetFromOrigin.distance > 50) {
                    controller?.setZoomLevel(
                      details.localOffsetFromOrigin.distance / 50,
                    );
                  }
                },
                onLongPress: () {
